...
function PlayerEditSelectChange() {
  var selectindex = +$('#PlayerEditNameList').val() - 1;
  var coloraddr = 0;
  ...
  if (selectindex < 117) {
    document.getElementById('PlayerModSelect')[
      NesHex[Edit_ModType + selectindex]
    ].selected = true;
  ...
  $('#PlayerSkincolour').css(
    'background-color',
    ReturnNesColor(NesHex[coloraddr]),
  );
  $('#PlayerHaircolor').css(
    'background-color',
    ReturnNesColor(NesHex[coloraddr + 1]),
  );
  $('#PlayerCoatcolor').css(
    'background-color',
    ReturnNesColor(NesHex[coloraddr + 2]),
  );
  $('#PlayerShortscolor').css(
    'background-color',
    ReturnNesColor(NesHex[coloraddr + 3]),
  );
...
function ReturnNesColor2(offset) {
  var c =
    '#' +
    nespal[offset * 3 + 0].toString(16) +
    nespal[offset * 3 + 1].toString(16) +
    nespal[offset * 3 + 2].toString(16);
  return c;
}
...
function getColoroptionsVal(op) {
  var ids = $(op).attr('id');
  $('#' + ids.substring(0, ids.length - 2)).css(
    'background-color',
    ReturnNesColor($(op).get(0).selectedIndex),
  );
  GetModAddrDataToString();
}
...
function GetModAddrDataToString() {
  ...
  texts +=
    addPreZero(
      $('#PlayerSkincolourSe').get(0).selectedIndex.toString(16),
    ).toUpperCase() + ' ';
  texts +=
    addPreZero(
      $('#PlayerHaircolorSe').get(0).selectedIndex.toString(16),
    ).toUpperCase() + ' ';
  texts +=
    addPreZero(
      $('#PlayerCoatcolorSe').get(0).selectedIndex.toString(16),
    ).toUpperCase() + ' ';
  texts +=
    addPreZero(
      $('#PlayerShortscolorSe').get(0).selectedIndex.toString(16),
    ).toUpperCase() + ' ';

  if (selectindex < 117) {
    texts +=
      addPreZero(
        $('#PlayerModSelect').get(0).selectedIndex.toString(16),
      ).toUpperCase() + ' ';
  }
...
function ChangeNowPlayerMod() {
  ...
  if (selectindex < 117) {
    NesHex[Edit_ModType + selectindex] =
      $('#PlayerModSelect').get(0).selectedIndex;
  }
...
function GetModFunction() {
  ...
  try {
    ...
    if (modtypes == -1) {
      modtypes = 18;
    } //if (modtypes >= 12) { defimg = "modimg/def2.png";}
    document.getElementById('canvasmodtemp').crossorigin = '';
    document.getElementById('canvasmodtemp').src = IMG_ModPic[modtypes];
    var post1 =
      nespal[colormod1 * 3 + 0] +
      ',' +
      nespal[colormod1 * 3 + 1] +
      ',' +
      nespal[colormod1 * 3 + 2];
    var post2 =
      nespal[colormod2 * 3 + 0] +
      ',' +
      nespal[colormod2 * 3 + 1] +
      ',' +
      nespal[colormod2 * 3 + 2];
    var post3 =
      nespal[colormod3 * 3 + 0] +
      ',' +
      nespal[colormod3 * 3 + 1] +
      ',' +
      nespal[colormod3 * 3 + 2];
    var post4 =
      nespal[colormod4 * 3 + 0] +
      ',' +
      nespal[colormod4 * 3 + 1] +
      ',' +
      nespal[colormod4 * 3 + 2];
...
function CGPNGColor(cor1, cor2, cor3, cor4) {
  ...
  for (var i = 0; i < CGdata.length; i += 4) {
    if (CGdata[i] == 0xff && CGdata[i + 1] == 0x00 && CGdata[i + 2] == 0xc8) {
      CGdata[i] = c1[0];
      CGdata[i + 1] = c1[1];
      CGdata[i + 2] = c1[2];
    }
    if (CGdata[i] == 0xff && CGdata[i + 1] == 0xe4 && CGdata[i + 2] == 0xd0) {
      CGdata[i] = c2[0];
      CGdata[i + 1] = c2[1];
      CGdata[i + 2] = c2[2];
    }
    if (CGdata[i] == 0x00 && CGdata[i + 1] == 0x00 && CGdata[i + 2] == 0xc8) {
      CGdata[i] = c3[0];
      CGdata[i + 1] = c3[1];
      CGdata[i + 2] = c3[2];
    }
    if (CGdata[i] == 0x00 && CGdata[i + 1] == 0x64 && CGdata[i + 2] == 0x00) {
      CGdata[i] = c4[0];
      CGdata[i + 1] = c4[1];
      CGdata[i + 2] = c4[2];
    }
  ...
  CGimg.src = document.getElementById('canvasmod').src =
...
function ChangGVsPKpic() {
  ...
  NesHex[playerVSmod_GK_addr + id2 * 2] = parseInt(
    $('#Vsmodelist2').find('option:selected').text().substring(0, 2),
    16,
  );
...
function bingDVSimg(type) {
  if (type == 1) {
    ...
    if (id == 0) {
      document.getElementById('VSmodIMG1').src =
      ...
      setTimeout(function () {
        document.getElementById('VSmodIMG1').src =
          document.getElementById('caVSmod').src;
      }, 200);
    }
  }
  if (type == 2) {
    ...
    switch (id) {
      case 0:
        document.getElementById('PlayerVSModColorList')[0].selected = true;
        break;
      case 1:
        document.getElementById('PlayerVSModColorList')[2].selected = true;
        break;
      case 2:
        document.getElementById('PlayerVSModColorList')[3].selected = true;
        break;
      case 3:
        document.getElementById('PlayerVSModColorList')[5].selected = true;
        break;
      case 4:
        document.getElementById('PlayerVSModColorList')[6].selected = true;
        break;
    }
    $('#PlayerVSModColorList').change();
    //document.getElementById("VSmodIMG2").src = document.getElementById("caVSmod").src;
    setTimeout(function () {
      document.getElementById('VSmodIMG2').src =
        document.getElementById('caVSmod').src;
    }, 200);
  }
}
...
function PlayerVSModColorChange() {
  ...
  document.getElementById('caVSmodtemp').src =
    IMG_VS_Pic[IMG_VS_PicNB[vspicindex]];
  ...
  if (vspicindex == 1 || vspicindex == 2) {
    document.getElementById('vscol_1')[
      NesHex[IMG_Name_Portrait[vspicindex][1]]
    ].selected = true;
    document.getElementById('vscol_2')[
      NesHex[IMG_Name_Portrait[vspicindex][2]]
    ].selected = true;
    document.getElementById('vscol_4')[
      NesHex[IMG_Name_Portrait[vspicindex][3]]
    ].selected = true;
    document.getElementById('vscol_3')[
      NesHex[IMG_Name_Portrait[vspicindex][4]]
    ].selected = true;
    ...
    document.getElementById('vscol_1')[
      NesHex[IMG_Name_Portrait[vspicindex][1] + xd]
    ].selected = true;
    document.getElementById('vscol_2')[
      NesHex[IMG_Name_Portrait[vspicindex][2] + xd]
    ].selected = true;
    document.getElementById('vscol_3')[
      NesHex[IMG_Name_Portrait[vspicindex][5] + xd]
    ].selected = true;
    document.getElementById('vscol_4')[
      NesHex[IMG_Name_Portrait[vspicindex][4] + xd]
    ].selected = true;
    document.getElementById('vscol_5')[
      NesHex[IMG_Name_Portrait[vspicindex][3] + xd]
    ].selected = true;
  } else if (
    vspicindex == 7 ||
    vspicindex == 8 ||
    ...
    document.getElementById('vscol_3')[
      NesHex[IMG_Name_Portrait[vspicindex][5]]
    ].selected = true;
    document.getElementById('vscol_4')[
      NesHex[IMG_Name_Portrait[vspicindex][4]]
    ].selected = true;
...
function PlayerVSModColorChange2() {
  ...
  setTimeout(function () {
    switch (vspicindex) {
      case 1:
        SetNewColor(idx_1, idx_2, idx_3, idx_4);
        break;
      case 2:
        SetNewColor(idx_1, idx_2, idx_3, idx_4);
        break;
      case 3:
        SetNewColor(idx_1, idx_2, 0x100, idx_3);
        break;
      case 7:
        SetNewColor(idx_1, idx_2, idx_3, idx_4);
        break;
      ...
      case 26:
        ...
        break;
      default:
        SetNewColor(idx_1, idx_2, idx_3, idx_4, idx_5);
        break;
    }
  }, 200);
}
...
function SetNewColor(
  ...
  CGimg.src = document.getElementById('caVSmod').src =
    CGcanvas.toDataURL('image/png');
}
...
function InputPlayerColorData(passdataid) {
  ...
  for (var i = 0x00; i <= 0x3f; i++) {
    ...
    list +=
      '<option  value=<span style="background-color:' +
      ReturnNesColor(i) +
      ';color:' +
      newcolor +
      ';">' +
